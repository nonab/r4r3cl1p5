...
$LastChangedDate: 2014-11-30 11:36:33 -0800 (Sun, 30 Nov 2014) $
$Rev: 12126 $
*/
...
function sp_ProfileShowUserPhotos($args='') {
	...
	if (!empty($spProfileUser->photos) || $showEmpty) {
		...
		$out = '';
		$out.= "<table class='$tagClass' style='width:100%'>";
		$col = 0;
		$width = 100 / $numCols;
        if (!empty($spProfileUser->photos)) {
    		foreach ($spProfileUser->photos as $photo) {
    			if ($col == 0) $out.= '<tr>';
    			$out.= "<td class='$photoClass' style='width:$width%'>";
    			if (!empty($photo)) {
            		$img = sp_get_image_size($photo, true);
                    $width = $height = $style = '';
                    if (!empty($img) && $img[0] > $sfprofile['photoswidth']) $width = "width:{$sfprofile['photoswidth']}px;";
                    if (!empty($img) && $img[1] > $sfprofile['photosheight']) $height = "height:{$sfprofile['photosheight']}px;";
                    if (!empty($width) || !empty($height)) $style = " style='$width$height'";
                    $out.= "<img class='$imageClass'$style src='$photo' />";
                }
    			$out.= '</td>';
    			$col++;
    			if ($col == $numCols) {
