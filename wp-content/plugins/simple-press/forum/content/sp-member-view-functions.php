...
function sp_MemberListUrl($args='', $label='') {
    ...
    if ($showIcon && !empty($icon) && $spThisMember->user_url != '') {
        ...
        $out.= "<a id='$tagId' class='$textClass' href='$spThisMember->user_url' title=''$target$follow><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' /></a>\n";
    } else {
...
function sp_MemberListActions($args='', $label='', $startedToolTip='', $postedToolTip='') {
	...
	if ($started) {
		...
		$out.= "<a href='".esc_url($url)."'><img class='$startedClass' src='$startedIcon' title='$startedToolTip' alt='' />$att</a>";
	}
	...
	if ($posted) {
		...
		$url = add_query_arg($param, sp_url());
		$url = sp_filter_wp_ampersand($url);
		$out.= "<a href='".esc_url($url)."'><img class='$postedClass' src='$postedIcon' title='$postedToolTip' alt='' />$att</a>";
	}
