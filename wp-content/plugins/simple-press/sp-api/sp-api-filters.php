...
function sp_filter_content_save($content, $action, $doEsc=true) {
	...
	$content = sp_filter_save_smileys($content);
...
# ------------------------------------------------------------------
function sp_filter_save_smileys($content) {
...
function sp_format_display_image($match) {
	...
	if ((int) $width[1] > (int)$thumb) { # is width > thumb size
		...
		$anchor = true;
	} else if (!empty($width[1]) && $width > 0) { # width is smaller than thumb, so use the width {
...
#	Version: 5.5.1
#	takes text content and returns and match snippets combined into an except
#
# --------------------------------------------------------------------------------------
function sp_filter_content_excerpt($text, $words, $size=100) {
    # find the search terms
    $s = '\s\x00-/:-@\[-`{-~';
    preg_match_all('#(?<=['.$s.']).{1,'.$size.'}(('.$words.').{1,'.$size.'})+(?=['.$s.'])#uis', $text, $matches, PREG_SET_ORDER);

    # add delimiter around the snippets
    $results = array();
    foreach ($matches as $line) {
        $results[] = '<br />&hellip;'.htmlspecialchars($line[0], 0, 'UTF-8');
    }
