...
$LastChangedDate: 2015-01-19 03:45:18 -0800 (Mon, 19 Jan 2015) $
$Rev: 12369 $
*/
...
function sp_TopicHeaderIcon($args='') {
	...
	if (!empty($spThisTopic->topic_icon)) {
		...
		$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($icon));
	}
...
function sp_TopicHeaderRSSButton($args='', $label='', $toolTip='') {
	...
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt=''/>";
...
function sp_PostNewButton($args='', $label='', $toolTip='', $toolTipLock='', $toolTipDenied='') {
	...
	if ($spGlobals['lockdown'] || $spThisTopic->forum_status || $spThisTopic->topic_status) {
		if (!empty($iconLock)) {
			$iconLock = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconLock));
			$out.= "<img class='$tagClass $iconStatusClass' id='$tagId' src='$iconLock' title='$toolTipLock' alt='' />\n";
		}
	...
	if (!$allowed && !empty($toolTipDenied)) {
		if (!empty($iconDenied)) {
			$iconDenied = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconDenied));
			$out.= "<a href='#' class='$tagClass' id='$tagId' title='$toolTipDenied'>\n";
			$out.= "<img class='$iconStatusClass' src='$iconDenied' title='$toolTipDenied' alt='' />\n";
			$out.= "</a>\n";
		}
	}
...
function sp_PostNewTopicButton($args='', $label='', $toolTip='', $toolTipLock='') {
	...
	if ($spGlobals['lockdown'] || $spThisTopic->forum_status) {
		if (!empty($iconLock)) {
			$iconLock = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconLock));
			$out.= "<img class='$tagClass $iconStatusClass' src='$iconLock' title='$toolTipLock' alt='' />\n";
		}
        if (!$spThisUser->admin) $lock = true;
	}
...
function sp_PostIndexPageLinks($args='', $label='', $toolTip='', $jumpToolTip='') {
	...
	# sanitize before use
	$tagClass		= esc_attr($tagClass);
	if (!empty($prevIcon)) $prevIcon	= sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($prevIcon));
	if (!empty($nextIcon)) $nextIcon	= sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($nextIcon));
	if (!empty($jumpIcon)) $jumpIcon	= sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($jumpIcon));
	$iconClass		= esc_attr($iconClass);
...
function sp_PostIndexPinned($args='', $toolTip='') {
	...
	if (!empty($icon)) $out.= "<img class='' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' title='$toolTip' alt='' />\n";
	$out.= "</span>";
...
function sp_PostIndexPermalink($args='', $label='', $toolTip='') {
	...
	$out = "<a class='$tagClass' id='$tagId' title='$toolTip' rel='nofollow' href='$spThisPost->post_permalink'>";
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt=''/>\n";
	if (!empty($label)) $out.= sp_filter_title_display($label);
	$out.= "</a>\n";
...
function sp_PostIndexEdit($args='', $label='', $toolTip='') {
	...
	$out.= "<a class='$tagClass' id='$tagId' title='$toolTip' rel='nofollow' href='javascript:document.usereditpost$spThisPost->post_id.submit();'>";
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt=''/>\n";
	if (!empty($label)) $out.= sp_filter_title_display($label);
	$out.= "</a>\n";
	$out.= '</form>'."\n";
...
function sp_PostIndexDelete($args='', $label='', $toolTip='') {
	...
	$out.= "<a class='$tagClass' id='$tagId' title='$toolTip' rel='nofollow' href='javascript:void(null)' onclick='spjDeletePost(\"$ajaxUrl\", $spThisPost->post_id, $spThisTopic->topic_id);'>";
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt=''/>\n";
	if (!empty($label)) $out.= sp_filter_title_display($label);
	$out.= "</a>\n";
...
function sp_PostIndexUserTwitter($args='', $toolTip='') {
	...
	if (!empty($icon)) {
		$out = "<a id='$tagId' class='$tagClass' href='$url' title='$toolTip'$target$follow><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' /></a>\n";
	}
...
function sp_PostIndexUserWebsite($args='', $toolTip='') {
    ...
    $follow = ($noFollow) ? ' rel="nofollow"' : '';
	if (!empty($icon)) {
		$out = "<a id='$tagId' class='$tagClass' href='$spThisPostUser->user_url' title='$toolTip'$target$follow><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' /></a>\n";
	}
...
function sp_PostIndexUserStatus($args='', $onlineLabel='', $offlineLabel='') {
	...
	if (($spThisUser->admin ||
        ...
        if (!empty($onlineIcon)) $icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($onlineIcon));
		$label = $onlineLabel;
		$labelClass = $onlineLabelClass;
		$status = true;
	} else {
		if (!empty($offlineIcon)) $icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($offlineIcon));
		$label = $offlineLabel;
			...
			if ($spThisUser->ID != $user->trackuserid) {
				if (!empty($spVars['pageview'])) {
					if ($spVars['pageview'] == 'topic' && $user->topic_id == $spVars['topicid']) {
						...
						if (($displayToAll) || ($spThisUser->admin) && ($includeAdmins) && (sp_is_forum_admin($user->trackuserid)) ||
						    ($displayToAll) || ($spThisUser->admin) && ($includeMods) && (sp_is_forum_mod($user->trackuserid)) ||
						    ($displayToAll) || ($spThisUser->admin) && ($includeMembers) && (!sp_is_forum_mod($user->trackuserid !=0))) {
