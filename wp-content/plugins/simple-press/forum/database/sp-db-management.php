...
Simple:Press
Desc:
$LastChangedDate: 2014-08-24 13:08:46 -0700 (Sun, 24 Aug 2014) $
$Rev: 11900 $
*/
...
function sp_save_edited_post() {
	...
	if (sp_get_auth('edit_any_post', $post->forum_id)) {
		...
		if ($post->user_id == $spThisUser->ID &&
			(sp_get_auth('edit_own_posts_forever', $post->forum_id) ||
			(sp_get_auth('edit_own_posts_reply', $post->forum_id) && $last_post))) {
			$canEdit = true;
		}
	}
	if (!$canEdit) {
        sp_notify(1, sp_text('Edit failed - you do not have permission'));
        return;
    }
