...
Simple:Press
Desc:
$LastChangedDate: 2015-01-23 21:42:21 -0800 (Fri, 23 Jan 2015) $
$Rev: 12393 $
*/
	...
	if(current_theme_supports('sp-theme-child-overlays')) {
		...
		if (!empty($parentTheme) && !empty($curTheme['color'])) {
			...
			if (file_exists(SPTHEMEBASEDIR.$curTheme['theme'].'/styles/overlays/'.$curTheme['color'].'.php')) {
				$color = SPTHEMEBASEDIR.$curTheme['theme'].'/styles/overlays/'.$curTheme['color'].'.php';
			} elseif(!empty($curTheme['color'])) {
				$color = SPTHEMEBASEDIR.$curTheme['parent'].'/styles/overlays/'.$curTheme['color'].'.php';
			}
		} else {
			if (!empty($curTheme['color'])) $color = SPTHEMEBASEDIR.$curTheme['theme'].'/styles/overlays/'.$curTheme['color'].'.php';
		}
	} else {
		# no support for child overlays so do it the old eway
		$color = (!empty($curTheme['color'])) ? $curTheme['color'] : '';
	}
	$vars = (!empty($color)) ? '?color='.esc_attr($color) : '';
...
function sp_forum_footer() {
	...
	<script type='text/javascript'>
		...
		jspf(document).ready(function() {
			...
			$docookie = false;
			if ($spVars['pageview'] == 'topic') $docookie = true;
			if ($docookie && sp_get_option('sfpostwrap')) { ?>
				var c = jspf(".spPostContentSection").width();
				if (c) {
					jspf.cookie('post_width_<?php echo($spDevice); ?>', c, { path: '/' });
				}
			<?php } ?>
