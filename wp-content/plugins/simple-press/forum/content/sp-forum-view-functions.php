...
function sp_SubForumIndexStatusIcons($args='', $toolTipLock='', $toolTipPost='', $toolTipAdd='') {
	...
	if ($spThisSubForum->forum_status == 1 || $spGlobals['lockdown'] == true) {
		$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconLock));
		$out.= "<img class='' src='$icon' title='$toolTipLock' alt='' />\n";
	}

	# New Post Popup
	if ($spThisSubForum->unread) {
		$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconPost));
		...
		$out.= "<img class='' src='$icon' title='$toolTipPost' alt='' /></a>\n";
	}
	...
	if (sp_get_auth('start_topics', $spThisSubForum->forum_id) && !$spThisSubForum->forum_status && !$spGlobals['lockdown']) {
		...
		$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconAdd));
...
function sp_ForumHeaderSubForums($args='', $label='', $toolTip='') {
	...
	foreach ($spThisForumSubs as $sub) {
		...
		if (($allNested == 0 && $sub->parent == $spThisSubForum->forum_id)
            ...
            if ($sub->unread) {
    			if (!empty($unreadIcon)) $tout.= "<img src='".sp_find_icon(SPTHEMEICONSURL, $unreadIcon)."' class='$iconClass' alt=''/>\n";
            } else {
    			if (!empty($icon)) $tout.= "<img src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' class='$iconClass' alt=''/>\n";
            }
...
function sp_TopicNewButton($args='', $label='', $toolTip='', $toolTipLock='', $toolTipDenied='') {
	...
	if ($spGlobals['lockdown'] || $spThisForum->forum_status) {
		if (!empty($iconLock)) {
			...
			$out.= "<img class='$tagClass $iconClass $statusClass' id='$tagId' src='$iconLock' title='$toolTipLock' alt='' />\n";
		}
        if (!$spThisUser->admin) $lock = true;
	}
    if (!$lock && sp_get_auth('start_topics', $spThisForum->forum_id)) {
		$out.= "<a href='javascript:void(null)' class='$tagClass' id='$tagId' title='$toolTip' onclick='spjOpenEditor(\"spPostForm\", \"topic\");'>\n";
		if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt=''/>\n";
		if (!empty($label)) $out.= sp_filter_title_display($label);
		$out.= "</a>\n";
	}
	...
	if (!$spThisForum->start_topics && !empty($toolTipDenied)) {
		...
		if (!empty($iconDenied)) {
			...
			$out.= "<img class='$iconClass' src='$iconDenied' title='$toolTipDenied' alt='' />\n";
			$out.= "</a>\n";
		}
	}
...
function sp_TopicIndexIcon($args='') {
	...
	if (!file_exists($path.$tIcon)) {
		$tIcon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($icon));
	} else {
		$tIcon = sp_find_icon($url, $tIcon);
	}
...
function sp_TopicIndexStatusIcons($args='', $toolTipLock='', $toolTipPin='', $toolTipPost='', $toolTipDenied='') {
	...
	if ($spGlobals['lockdown'] || $spThisTopic->topic_status) {
		if (!empty($iconLock)) {
			$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconLock));
			$out.= "<img class='$iconClass' src='$icon' title='$toolTipLock' alt='' />\n";
		}
	}

	if ($spThisTopic->topic_pinned) {
		if (!empty($iconPin)) {
			$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconPin));
			$out.= "<img class='$iconClass' src='$icon' title='$toolTipPin' alt='' />\n";
		}
	}
	...
	if ($spThisTopic->unread) {
		if (!empty($iconPost)) {
			$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconPost));
			$out.= "<img class='$iconClass' src='$icon' title='$toolTipPost' alt='' />\n";
		}
	}

	if (!$spThisTopic->reply_topics && !empty($toolTipDenied)) {
		$icon = sp_find_icon(SPTHEMEICONSURL, sanitize_file_name($iconDenied));
		$out.= "<img class='$iconClass' src='$icon' title='$toolTipDenied' alt='' />\n";
	}
