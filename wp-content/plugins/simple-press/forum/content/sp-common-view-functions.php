...
$LastChangedDate: 2014-12-21 19:33:21 -0800 (Sun, 21 Dec 2014) $
$Rev: 12211 $
*/
...
function sp_OpenCloseControl($args='', $toolTipOpen='', $toolTipClose='') {
	...
	$out = "<span id='$tagId' onclick='spjOpenCloseSection(\"$targetId\", \"$tagId\", \"$tagClass\", \"$openIcon\", \"$closeIcon\", \"$toolTipOpen\", \"$toolTipClose\");'>";
...
function sp_LogInOutButton($args='', $inLabel='', $outLabel='', $toolTip='') {
	...
	if (is_user_logged_in() == true) {
		if (!$mobileMenu) {
			$out.= "href='$logOutLink'>";
			if (!empty($logOutIcon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $logOutIcon)."' alt=''/>";
		...
		if (!$mobileMenu) {
			...
			if (!empty($logInIcon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $logInIcon)."' alt=''/>";
...
function sp_QuickLinksTopic($args='', $label='') {
	...
	if (!empty($spList->listData)) {
		...
		foreach ($spList->listData as $spPost) {
			...
			$class = 'spPostRead';
			$title = "title='".sp_find_icon(SPTHEMEICONSURL, "sp_QLBalloonNone.png")."'";
			if ($spPost->post_status != 0) {
				$class = 'spPostMod';
				$title = "title='".sp_find_icon(SPTHEMEICONSURL, "sp_QLBalloonRed.png")."'";
			} elseif (sp_is_in_users_newposts($spPost->topic_id)) {
				$class = 'spPostNew';
				$title = "title='".sp_find_icon(SPTHEMEICONSURL, "sp_QLBalloonBlue.png")."'";
			}
...
function sp_BreadCrumbs($args='', $homeLabel='') {
	...
	if (!empty($icon)) {
        $icon = "<img src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' class='$iconClass' alt=''/>";
    } else {
	...
	if (!empty($homeIcon)) {
        $homeIcon = "<img src='".sp_find_icon(SPTHEMEICONSURL, $homeIcon)."' class='$homeIconClass' alt=''/>";
    } else {
		...
		for ($x=0; $x<strlen($order); $x++) {
			...
			if ($unreads > 0 && $item != 'T') {
				if ($item == 'L') {
					...
					$out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, "$unreadIcon")."' title='$unreadToolTip' alt='' /></a>\n";
				}
				if ($item == 'M') {
					$out.= "<a href='javascript:void(null)' onclick='spjMarkRead(\"$ajaxUrl\");'><img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, "$markIcon")."' alt='' title='$markToolTip' /></a>";
				}
			}
		}
...
function sp_SearchForm($args='') {
	...
	$out.= "<a rel='nofollow' id='$submitId' class='$submitClass' title='$toolTip' onclick='spjValidateSearch(this, \"$submitId\", \"link\", {$spGlobals['mysql']['search']['min']});'>";
	if (!empty($icon)) {
...
function sp_GoToTop($args='', $label='', $toolTip='') {
	...
	if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' title='$toolTip' />";
	$out.= "$label</a>";
	$out.= "</div>\n";
...
function sp_Acknowledgements($args='', $label='', $toolTip='', $siteToolTip='') {
	...
	$out = "<div id='$tagId' class='$tagClass'>";
    $out .= "&copy; <a class='spLink' title='$siteToolTip' href='http://simple-press.com' target='_blank'>Simple:Press</a>";
	if ($showPopup) {
		$site = SFHOMEURL.'index.php?sp_ahah=acknowledge&amp;sfnonce='.wp_create_nonce('forum-ahah');
		$out.= "<a rel='nofollow' class='$linkClass' title='$toolTip' href='javascript:void(null)' onclick='spjDialogAjax(this, \"$site\", \"$toolTip\", 600, 0, \"center\");'>";
		if (!empty($icon)) $out.= "<img class='$iconClass' src='".sp_find_icon(SPTHEMEICONSURL, $icon)."' alt='' />";
		$out.= "$label</a>";
	}
    $out.= "</div>\n";
